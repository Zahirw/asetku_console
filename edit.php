...
{	
	...
	// update user data
	$result = mysqli_query($koneksi,"SELECT FROM_UNIXTIME(t_lender_user_ext.birth_date/1000, '%Y/%d/%M' ) birth_date,
                                    t_lender_user_ext.uid,t_user.first_name,
                                    t_user.phone_number,t_user.gender,
                                    t_lender_user_ext.age,t_lender_user_ext.religion,
                                    t_lender_user_ext.place_of_birth,
                                    t_lender_user_ext.address,t_lender_user_ext.city,
                                    t_lender_user_ext.province,t_country.country_name,
                                    t_user.occupation
                                    FROM t_lender_user_ext, t_user, t_country
                                    where t_lender_user_ext.uid  = t_user.uid and t_user.country_id  = t_country.id and t_lender_user_ext.uid= $_GET[id]");
	...
	while($hasil = mysqli_fetch_assoc($result))
	{
		...
		$cntry		=	ucfirst($hasil['country_name']);
